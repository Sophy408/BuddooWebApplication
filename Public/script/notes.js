...
async function saveNotesData() {
  try {
    // First get current data to preserve todos and events
    const currentRes = await fetch('/api/data', { credentials: 'include' });
    const currentData = currentRes.ok ? await currentRes.json() : { todos: {}, events: {} };
...
addCategoryBtn.addEventListener('click', () => {
  ...
  if (newCat && !categories.includes(newCat)) {
    ...
    categoryInput.value = '';
    saveNotesData(); // Save to server instead of localStorage
    renderCategories();
    renderNotes();
